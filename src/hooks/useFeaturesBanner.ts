  ...
  useEffect(() => {
    async function loadBanner() {
      ...
      try {
        ...
        if (isDemoProperty) {
          console.log('Loading demo features banner')
          ...
          for (const format of supportedFormats) {
            ...
            try {
              ...
              if (response.ok) {
                console.log(`Found demo features banner in ${format} format`)
                setImageUrl(publicUrlData.publicUrl)
                foundImage = true
                break
              }
            } catch (err) {
              console.log(`No ${format} format found for demo features banner:`, err)
            }
          }

          if (!foundImage) {
            console.error('No supported image format found for demo features banner')
            setImageUrl(null)
          }
          return
        }
